...
## 3.8.14
- UPD: Multiday calendar. Add "Current Week Day" styling controls and highlight the current day;
- FIX: Dynamic Calendar. Reinitialize Bricks widget scripts when opening mobile calendar events;
- FIX: Profile Builder. Listing dynamic CSS issue;
- FIX: Listing block styles are now properly inlined for AJAX-rendered listings;
- FIX: Listings. Prevent dynamic-visibility checks while preloading listing item assets to avoid duplicate checks and ensure assets are loaded;
- FIX: WooCommerce. Listing quantity handling and optional variation-to-WC_Product conversion;
- FIX: Dynamic Terms. Custom SVG icon is not rendered in Bricks;
- FIX: Taxonomies. Normalize taxonomy `object_type` values to support multi-object types and avoid registration issues;
- FIX: Maps Listings. Normalize map bounds on sync event to prevent errors in some cases;
- FIX: WPML compatibility. Translate taxonomy labels when registering JetEngine taxonomies

## 3.8.13.2
- FIX: Dynamic Calendar. Escape unsafe macros output before print in Multiday calendar 

## 3.8.13.1
- FIX: Prevent unsafe files uploads in Forms (Legacy) module (thanks to Nguyễn Đức Hòa).
